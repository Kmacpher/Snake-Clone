...
import edu.trinity.cs.gamecore.Player;
	 ...
	 * the Player dies and the game is over. Goomba entities are the same as Ghost entities, however they do
	 * not appear on easy. they appear together with the ghost entities during normal and hard play.
	 * @author kmacpher
	 *
	 */
	public class GoombaEntity implements SnakeEntity {

		/**
		 * Constructor takes the entity location and places it there on the screen.
		 * @param loc
		 */
		public GoombaEntity(Location<SnakeBlock,SnakeEntity> loc) {
	        location = loc;
		...
		public Image getImage() {
			if(img==null) {
				 try {
						img = ImageIO.read(new File("Goomba.png"));
					} catch (IOException e) {
